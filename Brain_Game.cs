...
Và mảng 2 chiều board có kích thước 10x10, dùng để lưu trữ trạng thái của các ô trên bảng.*/
        ...
        static int[,] board = new int[10, 10];
​​         // Tạo biến để theo dõi vị trí hiện tại và trước đó trên bảng, số lượt chơi và cấp độ của trò chơi.
...
Nếu có bất kỳ phần tử nào chưa được lật (có giá trị khác 1) thì phương thức trả về giá trị false, tức là trò chơi chưa kết thúc và ngược lại */
	...
	//  lưu các ký tự trong bảng
            ...
            for (int j = 0; j < gridSizes[i, 0]; j++)
                ...
                {
                    int x, y;
	        ​​/* Kiểm tra nếu count + 1 nhỏ hơn half, tức là số lượng ký tự đã được đặt vào bảng chưa đạt đến giá trị half, thì tiếp tục vòng lặp while để đặt ký tự vào bảng. */
                    if (count + 1 < half)
                        while (true)
		// Tạo một bảng trò chơi mới với các ký tự được đặt vào ngẫu nhiên
                        {
                            Random r = new Random();
                            x = r.Next(0, gridSizes[i, 0]);
                            y = r.Next(0, gridSizes[i, 1]);
                            int tmp = (cnt_symbol + 1) % 9;
	        	    ​​       /* Kiểm tra nếu phần tử tại vị trí (x, y) trong bảng đã được đặt ký tự, thì tiếp tục vòng lặp while để tạo một vị trí mới và đặt ký tự vào.
                    	    Ngược lại, thì tăng giá trị count lên 1, đặt ký tự vào phần tử này trong bảng, và lưu ký tự vào mảng tmp_arr tại vị trí count.
     Sau khi đã đặt đủ số lượng ký tự vào bảng thì kết thúc vòng lặp. */
            ...
            for (int j = 0; j < gridSizes[i, 0]; j++)
                for (int k = 0; k < gridSizes[i, 1]; k++)
	                ​​/* Trong mỗi vòng lặp, kiểm tra nếu giá trị tại vị trí (j, k) trên bảng board là -1(tức là vị trí đó chưa được điền giá trị),
 thì thực hiện các bước trong khối lệnh {}. */
/* giá trị từ mảng tmp_arr được điền vào bảng board theo đúng thứ tự và vị trí tương ứng.
 Sau đó giá trị của biến count giảm đi 1 để trỏ tới phần tử tiếp theo của mảng tmp_arr (--count). */
                    if (board[j, k] == -1)
                    {
